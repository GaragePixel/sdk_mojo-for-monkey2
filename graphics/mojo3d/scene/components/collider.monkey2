...
'Alias Axis:stdlib.math.coordinates
...
Class CylinderCollider Extends ConvexCollider
	...
	[jsonify=1]
	Property Axis:stdlib.math.coordinates.Axis()
		
		Return _axis
		
	Setter( axis:stdlib.math.coordinates.Axis )
	...
	Field _axis:stdlib.math.coordinates.Axis=stdlib.math.coordinates.Axis.Y
	Field _origin:Vec3f
...
Class ConvexHullCollider Extends ConvexCollider
	...
	Method OnCreate:btCollisionShape() Override
		...
		For Local i:=0 Until vertices.Length
			stdlib.plugins.libc.memcpy( points.Data+i*3,Varptr( vertices[i].position ),12 )
		Next
...
Class MeshCollider Extends ConcaveCollider
	...
	Method OnCreate:btCollisionShape() Override
		...
		For Local i:=0 Until vertices.Length
			stdlib.plugins.libc.memcpy( _vertices.Data+i*3,Varptr( vertices[i].position ),12 )
		Next
		
		Local indices:=_mesh.GetAllIndices()
		_indices=New Int[indices.Length]
		stdlib.plugins.libc.memcpy( _indices.Data,indices.Data,_indices.Length*4 )
