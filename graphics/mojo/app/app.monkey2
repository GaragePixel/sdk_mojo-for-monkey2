...
Namespace sdk_mojo.m2.app

Using sdk_mojo.m2.input

Using sdk_mojo.m2.audio
...
Class AppInstance
	...
	Method New()' Virtual
		...
		stdlib.plugins.libc.atexit( SDL_Quit )
...
#Endif
	...
	Property Millisecs:Int()
		...
		Return stdlib.system.time.Millisecs()
	End
	...
	Method Sleep( seconds:Double )
		...
		Repeat
			...
			If sleep>10
				stdlib.system.time.Sleep( sleep )
				UpdateWindows()
			Else If sleep>0
				stdlib.system.time.Sleep( sleep )
			Else
...
#Endif
	...
	#rem monkeydoc @hidden
	#end	
	Method OnMainLoop() Virtual
		' Modified/Added by iDkP
		' Added the Begin/On/End structure around the original Sibly's mainloop
		' That's allows to add extra functionnalities in an extended class of AppInstance
		' like the AppX derivated class who allow to handle full windowed gui.
		If Not _requestRender Or Not Renderable SDL_WaitEvent( Null )
	...
	#rem monkeydoc @hidden
	#end	
	Method BeginMainLoop() Virtual
		' Added by iDkP
	End

	#rem monkeydoc @hidden
	#end	
	Method EndMainLoop() Virtual
		' Added by iDkP
	End
...
'				Print "SDL_WINDOWEVENT_FOCUS_LOST"
		...
		Case SDL_RENDER_DEVICE_RESET
			...
			sdk_mojo.m2.graphics.glutil.glGraphicsSeq+=1
